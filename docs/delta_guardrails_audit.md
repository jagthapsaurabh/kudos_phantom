...
| Track `blocked_margin` / `available_balance` | ✅ | `normalize_balance` reads the venue's own `blocked_margin` and, when it is absent, sums every per-mode bucket (isolated `order_margin`/`position_margin`/`commission`, `cross_*`, `portfolio_margin`) — `broker_account.py:363-402/404`; `portfolio_risk` (`:534`) then reports utilisation against `meta.net_equity`. Anything the wallet withholds that no bucket names lands in `unattributed_margin` + `balances_reconciled` instead of reading as free cash, and `margin_preflight.check_affordable` refuses to start an instance the wallet cannot fund |
