...
| `POST` | `/admin/market-data/seed` | `source`, `symbol`, `intervals`, dates, `limit`, `fetch_all`, `repair` | Fetch and upsert OHLCV data with volume. Full-history (`fetch_all`) requests default to 1 Jan 2020 for every source, are split into API-safe windows (Binance 1,500 / Delta 2,000 candles) and include the daily `1d` candles. `repair: true` deletes duplicate and off-grid candles first. Delta full history excludes `1m`/`5m`. |
