...
| Method | Endpoint | Request Body | Description |
| :--- | :--- | :--- | :--- |
| `GET` | `/broker-definitions` | None | Enabled exchange/broker data sources (Binance, Delta India and Delta Global are built in) |
...
| `POST` | `/broker-connections/{id}/probe` | None | **Check key.** Signs one `GET /v2/wallet/balances` against **all four** Delta environments (India production/testnet, Global production/testnet) and says which one accepts this key: `{environment, family, accepted_by, rejected_by, unreachable_from_here, matches_connection, mismatch, permission_gap, summary, fix, rows, detected}`. A production key on a testnet-flagged connection, or an India connection holding a Global key, is reported as a repoint, not a re-created key; only when every answering host rejects it is the key itself declared dead. Runs from the server, which is the machine a whitelisted key trusts. When the probe succeeds the connection's cached `account_settings` refresh and running instances re-read it |
| `POST` | `/broker-connections/{id}/test` | None (`?apply=true`) | **Test connection.** Same read-only battery as `tools/test_connection.py`: public market data, clock skew, all four Delta environments, signed calls (balance / positions / open orders / order history / trading preferences, plus `GET /v2/sub_accounts` — the endpoint the margin-mode panel reads, reported as `accounts` and never decisive, because a sub-account key may not list its parent), rate quota. Every step carries `state` (`ok` / `permission` / `auth` / `unreachable` / `error`) and the `endpoint` it actually called, so a call that never left the server reads as `unreachable` + a `problems` entry instead of a passing verdict. Never places/edits/cancels an order. Returns `{steps, detected, verdict: {ok, problems, fixes, message}}`. With `?apply=true`, repoints the saved connection at the detected environment (broker code + testnet flag), re-reads account settings and hands the new credentials to running instances — the fix the report describes, no restart |
...
| `GET` | `/sessions` | `mode`, `strategy_id`, `exchange` (query), `limit` | Saved finished sessions (real-money **and** paper runs live in this one list). Every row carries a `mode` (`live`\|`paper`); pass `mode=live` or `mode=paper` to keep the two histories apart, as the **History** page's Live Trade History / Paper Trade History tabs do. `GET`/`DELETE /sessions/{id}` read/remove one row |
...
| `POST` | `/live-account/margin-mode` | `broker`, `connection_id`, `symbol`, `mode` (`isolated`\|`cross`), `subaccount_user_id` (optional) | Set the margin mode. On Delta India the venue requires a `subaccount_user_id`; when it is omitted and the connection key is Delta, the server resolves the key's *own* user id and sets that account. Pass a specific id to manage another account the key can administer |
| `POST` | `/live-account/margin-mode-all` | `broker`, `connection_id`, `symbol`, `mode` (`isolated`\|`cross`), `dry_run` | Set the margin mode on **every account visible to this key** (main account + every sub-account on Delta India). Returns `{status: ok\|partial\|rejected, response: {margin_mode, changed, total, results: [{id, account_name, is_sub_account, before, status, error}]}, rate_limits}` — `partial` when at least one account changed and at least one failed, each with the venue's error text. A key that cannot see other accounts (a sub-account key) is rejected with 400 naming the parent-key requirement. `dry_run: true` lists `targets` without writing |
| `POST` | `/live-account/position-margin` | `broker`, `connection_id`, `symbol`, `amount` | Add (positive) or remove (negative) isolated margin |
| `GET` | `/live-account/balance` | `broker` (query), `connection_id` | Wallet + margin for one account. Always answers 200 with a `state` the panel can render (`ok` / `no_credentials` / `error`): `wallet_balance`, `available_balance`, and `used_margin` — **everything the venue is holding back, in whichever margin mode holds it**. Delta blocks margin per mode (`order_margin` / `position_margin` / `commission` are isolated-only, `cross_order_margin` / `cross_position_margin` / `cross_commission` / `cross_locked_collateral` are cross, `portfolio_margin` is portfolio) with `available_balance = balance - blocked_margin`, so every bucket is reported individually plus the per-mode subtotals (`isolated_margin`, `cross_margin`, `portfolio_blocked`), the venue's own `blocked_margin` (`blocked_margin_reported` is null when we had to sum the buckets ourselves) and the `margin_mode` in play (`isolated` / `cross` / `portfolio` / `mixed`) with `margin_mode_source` saying where it came from — `blocked_margin` when a bucket is actually holding cash, `connection_settings` when the wallet is flat and the mode is the account's configured one (read from the connection's cached settings, no API call). `reserved_margin` (`wallet_balance - available_balance`) minus `used_margin` is `unattributed_margin`, with `balances_reconciled` saying whether the two agree — cash the venue withholds without naming a bucket is shown, never read as free. `net_equity` / `total` / `unrealized_pnl` come from the response envelope's `meta.net_equity` (wallet + unrealised PnL), and `balances` carries one row per asset |
